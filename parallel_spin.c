...
pthread_spin_lock(&spinlock); //acquire a lock, so no one else can change the table 
    e->next = table[i];
    e->key = key;
    e->val = val;
    table[i] = e;
pthread_spin_unlock(&spinlock); //release after its done
}

// Retrieves an entry from the hash table by key
// Returns NULL if the key isn't found in the table
bucket_entry * retrieve(int key) {
pthread_spin_lock(&spinlock); //acquire a lock, so no one else can change
    bucket_entry *b;
...
int main(int argc, char **argv) {
    ...
    pthread_t *threads;
    double start, end;
pthread_spin_init (&spinlock, 0); //initialize the lock
